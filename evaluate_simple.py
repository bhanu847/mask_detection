...
def draw_boxes_and_save(image_path, preds, out_path):
    ...
    for p in preds:
        ...
        label = f"{CLASS_NAMES[int(cls)]} {conf:.2f}" if int(cls) < len(CLASS_NAMES) else f"{int(cls)} {conf:.2f}"
        cv2.rectangle(img, (x1,y1), (x2,y2), (0,255,0), 2)
        cv2.putText(img, label, (x1, max(10,y1-5)), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,255), 1)
...
def main():
    ...
    if not val_imgs:
        print("[ERROR] No validation images found under", os.path.join(DATA_ROOT, "images", "val"))
        return
    ...
    for img_path in val_imgs:
        ...
        if gt:
            if preds:
                ...
                for gcls in gt:
                    gt_list.append(gcls)
                    pred_list.append(len(CLASS_NAMES))
        else:
            for p in preds:
    ...
    report_names = CLASS_NAMES + ["no_detection"]
    y_true = np.array(gt_list, dtype=int)
    y_pred = np.array(pred_list, dtype=int)

    print("[INFO] Annotated images saved to:", OUT_DIR)
    if len(y_true) == 0:
        print("[WARN] Nothing to evaluate (no gt and no preds).")
    else:
        print("\nClassification report (simple/object-level):\n")
        print(classification_report(y_true, y_pred, labels=list(range(len(report_names))), target_names=report_names, zero_division=0))
...
def main():
    ...
    for img_path in val_imgs:
        ...
        # Read ground truth labels (YOLO format)
        gt = []
        if os.path.exists(lbl_path):
            for L in open(lbl_path).read().splitlines():
                parts = L.strip().split()
        ...
        # Save qualitative results (annotated image)
        out_file = os.path.join(OUT_DIR, base + "_pred.jpg")
        draw_boxes_and_save(img_path, preds, out_file)
